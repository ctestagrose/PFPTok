...
def _as_seq_list(x):
    """Convert various tensor/array/list types to a nested Python list."""
    if x is None:
        return []
...
def attention_token_importance(
    ...
    Returns:
        token_scores: list[Tensor] length B, each (L_valid,) token importance
        gene_scores:  list[dict]   length B, mapping {gene_id_or_name: score}
    """
    model.eval()
