...
class GeneManager:
    ...
    def read_fasta_file_genes(self, file_path, target, filter_xs=False, sort_by_genes=True,
        ...
        """
        Reads a FASTA file and extracts gene and intergenic (IR) sequences.

        Args:
            method: Controls how sequences are combined:
                "no_intergenic" - Only gene sequences, no IR sequences.
                "merged" - Each gene is concatenated with its before/after IR regions.
                "separate" - Original: [IR_before, gene, IR_after, IR_before, gene, IR_after, ...]
        """
        if use_gene_file:
    ...
    def read_fasta_file_genes_all(self, file_path, target=None, filter_xs=False):
        """Read all genes from a FASTA file (not filtered by gene file)."""
